...
class BGFManager:
    ...
    def find_bgf_for_monster(self, class_name):
        ...
        # Determine if it's already a bgf filename or a class name
        if class_name.lower().endswith('.bgf'):
            ...
            candidates = [
                f"{class_name}.bgf",
                f"{class_name.lower()}.bgf",
                f"{class_name[:8]}.bgf",
                f"{class_name[:8].lower()}.bgf"
            ]
        ...
        for d in search_dirs:
            ...
            for c in candidates:
                p = os.path.join(d, c)
                if os.path.exists(p):
                    logger.info(f"BGF: Found exact match at {p}")
                    return p
            ...
            try:
                ...
                for f in files:
                    if f.lower() in [c.lower() for c in candidates]:
                        logger.info(f"BGF: Found case-insensitive match at {os.path.join(d, f)}")
